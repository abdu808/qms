/**
 * services/consultantAgent.js — موجه الأداء والجودة الذكي (v3)
 *
 * التغييرات عن v1:
...
قواعد التوجيه:
  • الخطة الاستراتيجية 2025-2027 معتمدة؛ لا تحاكم اعتمادها ولا تهدمها.
  • قيّم منهجية التنفيذ وجاهزية المتابعة فقط: المؤشرات، الأنشطة، الملاك، البيانات، التنبيهات، التقارير.
  • عدم وجود Objective ليس مشكلة بذاته، ولا يخفض جاهزية التنفيذ.
...
عند طلب تقييم الخطة، اعتبر المقصود تقييم جاهزية تنفيذها ومنهجية متابعتها، لا صلاحية اعتمادها.
استخدم خريطة الترابط أو أداة evaluate_strategic_plan، ولا تستخدم معيار "0 Objective" كدليل ضعف.
`;
...
function isPlanEvaluationRequest(text = '') {
  ...
  const asksEvaluation = /(تقييم|تقيم|تقيمم|قيّم|قيم|قيمم|راجع|مراجعة|احكم|المنهجية|منهجية|الحديثة|الحديده|تنفيذ|جاهزية|جاهزيه)/.test(normalized);
  const asksPlan = /(الخطة|خطة|استراتيجي|استراتيجية|الاستراتيجية|المؤشرات|الأهداف|الاهداف|الأنشطة|الانشطة|المتابعة|الاداء|الأداء)/.test(normalized);
...
function formatExecutionReadinessReply(planMap) {
  ...
  return `أعدت قراءة الخطة من زاوية التنفيذ والتحسين، لا من زاوية الحكم على اعتمادها.

**جاهزية التنفيذ والمتابعة: ${s.score}/100 - ${label}**

منهجية العمل:
- الخطة 2025-2027 معتمدة ولا أعيد الحكم على صلاحيتها.
- أراجع ما يساعد على نجاحها: المؤشرات، الأنشطة، الملاك، مدخلو البيانات، المستهدفات، التنبيهات، والتقارير.
- لا أطلب منك اختيار منهجية، ولا أعاقب الخطة بسبب غياب Objective؛ هذه طبقة اختيارية/قديمة.
...
الخلاصة: المطلوب ليس هدم الخطة أو إعادة تقييم اعتمادها، بل تحسين تشغيلها. الأولوية تكون لسد المشكلات أعلاه: تثبيت الملاك، إكمال المستهدفات، ربط المؤشرات والأنشطة حيث يلزم، وضبط المتابعة الدورية والتنبيهات.`;
}

const BASE_SYSTEM_PROMPT = `أنت "موجه الأداء والجودة" لجمعية بر خيرية تطبِّق ISO 9001:2015.

دورك: مساعد مدير الاستراتيجية والجودة. لا تهدم الخطة المعتمدة ولا تحاكمها؛ ساعد الإدارة والموظفين على تنفيذها وتحسين المتابعة.
...
عند طلب "قيّم الخطة":
  ...
  3. أعد صياغة الطلب داخلياً كالتالي: "قيّم جاهزية تنفيذ الخطة ومنهجية المتابعة".
  4. لا تسأل المستخدم أي منهجية يقصد، ولا تعرض قائمة OKR/BSC/IPMS.
  5. لا تعتمد على معيار 0 Objective ولا تكتب أن الترابط معطل إلا إذا أظهرت الخريطة ذلك.
  6. اعرض درجة جاهزية التنفيذ مع أسبابها من المشكلات والتنبيهات الفعلية في الخريطة.
...
قواعد العمل:
  ...
  • عند تقييم الخطة شغّل evaluate_strategic_plan أو استخدم planConnectivity من get_system_state، واجعل العنوان "جاهزية التنفيذ والمتابعة".
  • لا تصدر درجة رقمية للخطة المعتمدة نفسها من الانطباع العام أو من عدد Objectives.
...
[نهاية الذاكرة — المحادثة الجديدة تبدأ الآن]`,
  ...
  if (isPlanEvaluationConversation(messagesWithMemory)) {
    ...
    return {
      reply: formatExecutionReadinessReply(planMap),
