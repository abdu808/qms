...
router.get('/requirements', requireAction('iso-readiness', 'read'), asyncHandler(async (req, res) => {
  ...
  const ownerHintsByPage = {
    swot: 'وحدة الاستراتيجية والجودة',
    interestedParties: 'وحدة الاستراتيجية والجودة',
    qualityScope: 'وحدة الاستراتيجية والجودة',
    processes: 'وحدة الاستراتيجية والجودة مع ملاك العمليات',
    organizationalChart: 'الإدارة التنفيذية / الموارد البشرية',
    qualityPolicy: 'الإدارة التنفيذية / الجودة',
    competence: 'الموارد البشرية',
    risks: 'مالك الخطر + وحدة الجودة',
    indicators: 'وحدة الاستراتيجية والجودة',
    operationalActivities: 'مالك النشاط / الإدارة المختصة',
    kpiTracking: 'مالك المؤشر + مدخل البيانات',
    training: 'الموارد البشرية / المعهد والتدريب',
    communication: 'الاتصال المؤسسي',
    documents: 'وحدة الجودة + مالك الوثيقة',
    ackDocuments: 'وحدة الجودة + الموارد البشرية',
    beneficiaries: 'الخدمة المجتمعية',
    suppliers: 'المشتريات / المالية / الجهة المستفيدة',
    ncr: 'مالك الحالة + وحدة الجودة',
    surveys: 'وحدة الجودة + مالك الخدمة',
    audits: 'وحدة الجودة',
    managementReview: 'الإدارة التنفيذية + وحدة الجودة',
    capa: 'مالك الإجراء + وحدة الجودة',
    improvementProjects: 'مالك التحسين + وحدة الجودة',
  };

  const simpleByGroup = {
    'السياق': 'فهم واقع الجمعية والجهات المؤثرة عليها، ثم تحويله إلى قرارات أو مخاطر أو فرص متابعة.',
    'القيادة': 'إثبات أن الإدارة حددت التوجه والمسؤوليات وأبلغت الموظفين بما يخص الجودة.',
    'التخطيط': 'تحويل المخاطر والأهداف إلى أعمال قابلة للمتابعة بمسؤول وموعد ودليل.',
    'الدعم': 'توفير ما يحتاجه العمل: كفاءة، تدريب، تواصل، وثائق، وإقرارات واضحة.',
    'التشغيل': 'إثبات أن الخدمة تُنفذ بطريقة مضبوطة ويمكن تتبعها من الطلب حتى الإغلاق.',
    'تقييم الأداء': 'قياس النتائج بانتظام: مؤشرات، رضا، تدقيق، ومراجعة إدارة.',
    'التحسين': 'تسجيل المشكلات وفرص التحسين ومعالجتها ثم التحقق من أن الحل فعّال.',
  };

  const evidenceExamplesByPage = {
    swot: ['سجل SWOT', 'محضر مراجعة السياق', 'تحويل بند مهم إلى خطر أو فرصة'],
    interestedParties: ['مصفوفة أصحاب المصلحة', 'احتياجات وتوقعات كل طرف', 'آلية متابعة'],
    qualityScope: ['وثيقة النطاق', 'حدود التطبيق', 'الاستثناءات إن وجدت'],
    processes: ['خريطة عمليات', 'مالك العملية', 'مدخلات ومخرجات'],
    organizationalChart: ['هيكل تنظيمي معتمد', 'وصف مسؤوليات', 'قرار اعتماد'],
    qualityPolicy: ['سياسة جودة معتمدة', 'إثبات نشرها', 'إقرارات الموظفين'],
    competence: ['وصف وظيفي', 'مصفوفة كفاءة', 'سجل فجوات تدريب'],
    risks: ['سجل خطر/فرصة', 'خطة معالجة', 'تاريخ مراجعة'],
    indicators: ['مؤشر مع مالك', 'مستهدف سنوي', 'آلية قياس'],
    operationalActivities: ['نشاط مرتبط بهدف', 'مسؤول وموعد', 'نسبة إنجاز'],
    kpiTracking: ['قراءات شهرية/ربع سنوية', 'سبب انحراف', 'إجراء تصحيحي عند الحاجة'],
    training: ['خطة تدريب', 'حضور', 'تقييم أثر'],
    communication: ['خطة اتصال', 'جمهور مستهدف', 'قناة وتكرار'],
    documents: ['وثيقة معتمدة', 'رقم إصدار', 'تاريخ مراجعة'],
    ackDocuments: ['نموذج إقرار', 'قائمة مستلمين', 'تاريخ إقرار'],
    beneficiaries: ['ملف مستفيد', 'طلب خدمة', 'قرار أهلية أو صرف'],
    suppliers: ['قائمة موردين', 'تقييم مورد', 'محضر استلام أو فحص'],
    ncr: ['بلاغ عدم مطابقة', 'إجراء احتواء', 'قرار معالجة'],
    surveys: ['استبيان رضا', 'عدد الردود', 'تحليل نتيجة'],
    audits: ['خطة تدقيق', 'قائمة تحقق', 'تقرير تدقيق'],
    managementReview: ['أجندة اجتماع', 'محضر مراجعة', 'قرارات متابعة'],
    capa: ['إجراء تصحيحي', 'سبب جذري', 'تحقق فعالية'],
    improvementProjects: ['مقترح تحسين', 'خطة تنفيذ', 'نتيجة متابعة'],
  };

  const buildRequirement = (item) => ({
    ...item,
    sourceChecklist: sourceChecklistName,
    simpleExplanation: item.simpleExplanation || simpleByGroup[item.group] || 'متطلب رسمي يحتاج دليلاً عملياً يثبت تطبيقه داخل الجمعية.',
    evidenceExamples: item.evidenceExamples || evidenceExamplesByPage[item.systemPage] || item.documentHints || [],
    ownerHint: item.ownerHint || ownerHintsByPage[item.systemPage] || 'مالك العملية المختص + وحدة الجودة',
    approvalNote: item.approvalNote || (['documents', 'qualityPolicy', 'qualityScope', 'organizationalChart'].includes(item.systemPage)
      ? 'يفضل أن يكون الدليل معتمداً برقم/تاريخ قبل اعتباره نهائياً.'
      : 'يكفي وجود سجل واضح ومحدث، ويُعتمد عند الحاجة حسب نوع الدليل.'),
